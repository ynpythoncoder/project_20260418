...
def read_zip_root_dir_name(zip_path: Path) -> str:
    with zipfile.ZipFile(zip_path, "r") as zip_file:
        ...
        for member in zip_file.namelist():
            ...
            if not normalized:
                continue
            top_level_names.add(normalized.split("/", 1)[0])

    if len(top_level_names) != 1:
        raise ValueError(
            "ZIP内の最上位ディレクトリが一意ではありません。"
            f" 検出結果: {sorted(top_level_names)}"
        )
...
def validate_project_identity(zip_path: Path, extracted_root_name: str) -> tuple[Path, str]:
    project_root = get_runtime_project_root()
    runtime_project_name = project_root.name
    zip_stem = zip_path.stem

    if runtime_project_name != zip_stem or runtime_project_name != extracted_root_name:
        raise ValueError(
            "構造展開を中止しました。\n"
            "\n"
            f"理由:\n"
...
def extract_zip_to_temporary_dir(zip_path: Path) -> tuple[tempfile.TemporaryDirectory[str], Path]:
    ...
    extracted_root = temp_root / read_zip_root_dir_name(zip_path)
    if not extracted_root.exists():
        temp_dir.cleanup()
        raise FileNotFoundError("ZIP展開後に最上位ディレクトリが見つかりませんでした。")
...
def find_candidate_apps(extracted_project_root: Path, runtime_project_root: Path) -> list[CandidateApp]:
    ...
    for layer_dir in iter_layer_dirs(extracted_project_root):
        for child_dir in sorted(layer_dir.iterdir()):
            if not child_dir.is_dir():
                continue
            if child_dir.name.startswith("."):
                continue
            if child_dir.name == "__pycache__":
                continue
